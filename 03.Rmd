...
-   Evaluate the **relative importance of variables** in a multiple
    linear regression.
...
Recall the `Advertising` data from [Chapter 2](#learning). Here are a
few important questions that we might seek to address:

1.  **Is there a relationship between advertising budget and sales?**
2.  **How strong is the relationship between advertising budget and
    sales?** Does knowledge of the advertising budget provide a lot of
    information about product sales?
3.  **Which media are associated with sales?**
4.  **How large is the association between each medium and sales?** For
    every dollar spent on advertising in a particular medium, by what
    amount will sales increase?
5.  **How accurately can we predict future sales?**
6.  **Is the relationship linear?** If there is approximately a
    straight-line relationship between advertising expenditure in the
    various media and sales, then linear regression is an appropriate
    tool. If not, then it may still be possible to transform the
    predictor or the response so that linear regression can be used.
7.  **Is there synergy among the advertising media?** Or, in stats
    terms, is there an interaction effect?

## Simple Linear Regression: Definition

**Simple linear regression:** Very straightforward approach to
predicting response $Y$ on predictor $X$.
...
-   $\beta_{1}$ = slope: average increase in Y associated with a
...
$\beta_{0}$ and $\beta_{1}$ are unknown constants (also known as model
coefficients or parameters).

Training data can produce estimates:
$$\hat{\beta}_{0}  and  \hat{\beta}_{1}$$

that can then predict future sales (Y) on the basis of particular value
of TV advertising budget (x)
...
The hat symbol denotes an estimated value or predicted value of the
response.

## Estimating Coefficients

-   Could also be thought of as "finding the best values of the
    parameters/ constants)

-   Data is used to estimate the coefficients $\beta_{0}$ and
    $\beta_{1}$
...
-   **Residual: Discrepancy between the actual outcome and the predicted
    outcome**
...
-   Next slide: each residual is the distance of each point from the
    line ("of best fit") and the total squared distance of all points to
    be as small as possible (ideally), such that the line is close to
    the points (least squares line).
...
-   Errors of the slope and intercept: To determine the precision of the
    estimates
...
-   For slope,$\hat{\beta}_{1}$ sigma-squared : noise or the variance of
    the errors around the line, spread of x's around mean. In a
    nutshell, the more noise around the line, the less precise the slope
    and the more spread out the x's (think : values on the x-axis), the
    more precise the slope.
...
-   **95% confidence interval:** a range of values such that with 95%
    ...
    parameter
    -   If we take repeated samples and construct the confidence
        interval for each sample, 95% of the intervals will contain the
